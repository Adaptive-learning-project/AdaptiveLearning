...
export const getLearningModules = (_params?: { category?: string }) => {
  ...
  return apiClient.get("/api/student/topics").then((res) => {
...
export const getLearningModule = (
  ...
  return apiClient.get(`/api/learning-modules/${encodeURIComponent(moduleId)}`, {
...
export const getMastery = (studentId: string) => {
  return apiClient.get(`/api/mastery/${encodeURIComponent(studentId)}`).then(
    (res) => ({ data: res.data })
  );
};
